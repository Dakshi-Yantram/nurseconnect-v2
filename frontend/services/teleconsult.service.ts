...
export const teleconsultService = {
  ...
  start: (bookingId: string) =>
    api.post<TeleConsultOut>('/teleconsult/start', { booking_id: bookingId }),

  myQueue: (stage?: TeleConsultStage) =>
    api.get<TeleConsultOut[]>('/teleconsult/queue', { params: stage ? { stage } : undefined }),

  /** Advances waiting/diet_review -> patient_assessment. */
  submitDiet: (id: string, dietNotes: string) =>
    api.patch<TeleConsultOut>(`/teleconsult/${id}/diet`, { diet_notes: dietNotes }),
  ...
  submitPatientIssues: (id: string, allOkay: boolean, issues?: string) =>
    api.patch<TeleConsultOut>(`/teleconsult/${id}/patient-issues`, {
      all_okay: allOkay,
      issues: allOkay ? null : issues,
    }),

  /** Marks the consultation completed — the backend requires an e-Rx already linked. */
  complete: (id: string) => api.patch<TeleConsultOut>(`/teleconsult/${id}/complete`, {}),
};
